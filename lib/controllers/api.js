...
router.post('/export/:fileNamePart', function (req, res) {
  var fileName = `hud-fema-data-${req.params.fileNamePart}`
  res.setHeader('Content-disposition', `attachment; filename="${fileName}.csv"`)
  var disasterNumbers = req.body.disasterNumbers
  if (!disasterNumbers) res.status(406).send('No disaster numbers sent. Not Acceptable.')
